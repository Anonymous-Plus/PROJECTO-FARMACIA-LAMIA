...
<head>
    ...
    <title>Farmácia Lâmia | Painel Atendente</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn { padding: 0.5rem 1rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; cursor: pointer; border: none; transition: var(--transition); }
        ...
        @media (max-width: 992px) { .sidebar { left: -260px; } .sidebar.open { left: 0; } .main-content { margin-left: 0; } .menu-toggle { display: block; } }
        @media (max-width: 576px) { .page-content { padding: 1rem; } .cards-grid { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        <header class="topbar">
            <div class="topbar-left"><button class="menu-toggle" id="menuToggle"><i class="bi bi-list"></i></button></div>
            <div class="topbar-right">
                <div class="topbar-notification"><i class="bi bi-bell"></i><span class="badge"><?php echo $stockBaixo; ?></span></div>
        ...
        <div class="page-content">
            ...
            <div class="cards-grid">
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $totalMedicamentos; ?></div><div class="card-label">Medicamentos Disponíveis</div></div><div class="card-icon medicamentos"><i class="bi bi-capsule"></i></div></div>
                ...
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $totalClientes; ?></div><div class="card-label">Total Clientes</div></div><div class="card-icon clientes"><i class="bi bi-people-fill"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $stockBaixo; ?></div><div class="card-label">Stock Baixo</div></div><div class="card-icon stock"><i class="bi bi-exclamation-triangle"></i></div></div>
            </div>

            <div class="table-container">
                <div class="table-header"><h3><i class="bi bi-exclamation-triangle-fill" style="color:var(--danger);"></i> Medicamentos com Stock Baixo</h3></div>
                <table><thead><tr><th>Nome</th><th>Stock</th><th>Mínimo</th><th>Status</th></tr></thead>
                    <tbody>
                        <?php $c = 0; foreach($medicamentos as $m): if($m->getQuantidadeEstoque() < $m->getEstoqueMinimo() && $c < 5): $c++; ?>
                        <tr><td><strong><?php echo htmlspecialchars($m->getNome()); ?></strong></td><td><?php echo $m->getQuantidadeEstoque(); ?></td><td><?php echo $m->getEstoqueMinimo(); ?></td><td><span class="badge-status <?php echo $m->getQuantidadeEstoque()==0?'badge-danger':'badge-warning'; ?>"><?php echo $m->getQuantidadeEstoque()==0?'Esgotado':'Baixo'; ?></span></td></tr>
                        <?php endif; endforeach; if($c==0): ?>
                        <tr><td colspan="4" style="text-align:center;padding:2rem;color:var(--success);"><i class="bi bi-check-circle-fill" style="font-size:2rem;display:block;margin-bottom:0.5rem;"></i>Todos os medicamentos com stock adequado!</td></tr>
                        <?php endif; ?>
                    </tbody></table>
            </div>
        </div>
    ...
    <!-- MODAL LOGOUT -->
    <div class="modal-overlay" id="modalLogout"><div class="modal" style="text-align:center;">
        <div class="modal-body"><div style="font-size:3rem; color:var(--danger); margin-bottom:1rem;"><i class="bi bi-box-arrow-right"></i></div><h4 style="font-size:1.2rem; margin-bottom:0.5rem; color:var(--text-dark);">Terminar Sessão</h4><p style="color:var(--text-muted); font-size:0.9rem;">Tem certeza que deseja sair?</p>
        <div style="display:flex; gap:1rem; justify-content:center; margin-top:2rem;"><button class="btn" onclick="document.getElementById('modalLogout').classList.remove('active')" style="background:#e9ecef; color:var(--text-dark);">Cancelar</button><a href="../../logout.php" class="btn btn-primary" style="background:var(--danger); text-decoration:none;"><i class="bi bi-box-arrow-right"></i> Sair</a></div></div>
    </div></div>
