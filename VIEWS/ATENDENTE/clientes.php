...
<head>
    ...
    <title>Farmácia Lâmia | Clientes</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:var(--bg);display:flex;min-height:100vh}
        ...
        .btn-danger{background:var(--danger);color:#fff}.btn-warning{background:var(--warning);color:#000}.btn-xs{padding:0.3rem 0.6rem;font-size:0.75rem}
        ...
        .modal{background:#fff;border-radius:20px;width:100%;max-width:550px;box-shadow:0 25px 60px rgba(0,0,0,0.3);transform:translateY(30px) scale(0.95);transition:all 0.4s cubic-bezier(0.175,0.885,0.32,1.275);max-height:90vh;overflow-y:auto}
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav"><ul>
            ...
            <li><a href="clientes.php" class="active"><i class="bi bi-people-fill"></i> Clientes</a></li>
        </ul></nav>
    ...
    <div class="main-content">
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="topbar-search"><i class="bi bi-search"></i><input type="text" placeholder="Pesquisar clientes..." id="buscaTopo"></div>
            </div>
        ...
        <div class="page-content">
            <div class="page-header">
                <div><h1><i class="bi bi-people-fill"></i> Clientes</h1><p>Gerir clientes da farmácia</p></div>
            ...
            <div class="table-container">
                ...
                <table><thead><tr><th>Nome</th><th>Sexo</th><th>Telefone</th><th>Email</th><th>Ações</th></tr></thead>
                    <tbody id="tabelaBody">
                        ...
                        <tr data-id="<?php echo $c->getIdCliente(); ?>">
                            ...
                            <td class="acoes">
                                <button class="btn btn-warning btn-xs" onclick="abrirModalEdicao(<?php echo $c->getIdCliente(); ?>)"><i class="bi bi-pencil"></i></button>
                        ...
                        <tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--text-muted);"><i class="bi bi-inbox" style="font-size:2rem;display:block;margin-bottom:0.5rem;"></i>Nenhum cliente cadastrado.</td></tr>
                        <?php endif; ?>
                    </tbody></table>
            </div>
        </div>
    ...
    <div class="modal-overlay" id="modalCadastro"><div class="modal">
        <div class="modal-header"><h3 id="modalTitulo">Novo Cliente</h3><button class="modal-close" onclick="fecharModal('modalCadastro')"><i class="bi bi-x-lg"></i></button></div>
        <div class="modal-body"><form id="formCliente">
            <input type="hidden" id="clienteId">
            <div class="form-row"><div class="form-group"><label>Nome <span style="color:var(--danger);">*</span></label><input type="text" id="nome" required></div><div class="form-group"><label>Sexo</label><select id="sexo"><option value="">Selecionar</option><option value="Masculino">Masculino</option><option value="Feminino">Feminino</option></select></div></div>
            <div class="form-row"><div class="form-group"><label>Telefone</label><input type="text" id="telefone"></div><div class="form-group"><label>Email</label><input type="email" id="email"></div></div>
            <div class="form-group"><label>Endereço</label><input type="text" id="endereco"></div>
        </form></div>
        <div class="modal-footer"><button class="btn" onclick="fecharModal('modalCadastro')" style="background:#e9ecef;color:var(--text-dark);">Cancelar</button><button class="btn btn-primary" onclick="salvarCliente()"><i class="bi bi-check-lg"></i> Salvar</button></div>
    </div></div>
    ...
    <script>
        ...
        async function abrirModalEdicao(id){
            try {
                const resposta = await fetch(API_CLIENTE, {
                    ...
                    body: JSON.stringify({action: 'buscar', idCliente: id})
                });
                const dados = await resposta.json();
                if (!dados.success) throw new Error(dados.message || 'Não foi possível carregar o cliente.');
                ...
                mostrarToast(erro.message || 'Erro ao carregar cliente.', true);
            }
        }
        ...
        async function salvarCliente(){
            ...
            const payload={
                action: clienteEmEdicao ? 'atualizar' : 'cadastrar',
                idCliente: clienteEmEdicao || '',
                nome,
            ...
            try {
                ...
                const dados = await resposta.json();
                if (!dados.success) throw new Error(dados.message || 'Não foi possível salvar o cliente.');
                fecharModal('modalCadastro');
                mostrarToast(dados.message || 'Cliente salvo com sucesso!');
        ...
        async function excluirCliente(id){
            try {
                const resposta = await fetch(API_CLIENTE, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({action: 'apagar', idCliente: id})
                });
                const dados = await resposta.json();
                if (!dados.success) throw new Error(dados.message || 'Não foi possível excluir o cliente.');
                fecharModal('modalExclusao');
                mostrarToast(dados.message || 'Cliente excluído com sucesso!');
