...
if ($nivel !== 'Administrador') { header('Location: index.php'); exit; }
$paginaAtual = basename($_SERVER['PHP_SELF']); $titulo = 'Utilizadores';
require_once '../../CONTROLLER/UtilizadorController.php';
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-admin { background: #d1ecf1; color: #0c5460; } .badge-farma { background: #d4edda; color: #155724; } .badge-atendente { background: #fff3cd; color: #856404; }
        .badge-ativo { background: #d4edda; color: #155724; } .badge-inativo { background: #f8d7da; color: #721c24; }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li><a href="utilizadores.php" class="active"><i class="bi bi-shield-lock"></i> Utilizadores</a></li>
    ...
    <div class="main-content">
        ...
        <div class="page-content">
            <div class="page-header">
                <div>
                    <h1><i class="bi bi-shield-lock"></i> <?php echo $titulo; ?></h1>
                    <p>Gerir contas de acesso ao sistema</p>
                </div>
                <button class="btn btn-primary" onclick="abrirModalCadastro()"><i class="bi bi-plus-lg"></i> Novo Utilizador</button>
            </div>

            <div class="table-container">
                <div class="table-header">
                    <h3>Todos os Utilizadores (<?php echo count($lista); ?>)</h3>
                </div>
                <table>
                    <thead>
                        <tr><th>Username</th><th>Nível</th><th>Estado</th><th>ID Funcionário</th><th>Ações</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach($lista as $u): 
                            $nivelBadge = $u->getNivel() == 'Administrador' ? 'badge-admin' : ($u->getNivel() == 'Farmaceutico' ? 'badge-farma' : 'badge-atendente');
                            $estadoBadge = $u->getEstado() == 'Ativo' ? 'badge-ativo' : 'badge-inativo';
                        ?>
                        <tr>
                            <td><strong><?php echo htmlspecialchars($u->getUsername()); ?></strong></td>
                            <td><span class="badge <?php echo $nivelBadge; ?>"><?php echo $u->getNivel(); ?></span></td>
                            <td><span class="badge <?php echo $estadoBadge; ?>"><?php echo $u->getEstado(); ?></span></td>
                            <td>#<?php echo $u->getIdFuncionario(); ?></td>
                            <td class="acoes">
                                <button class="btn btn-success btn-xs" title="Alterar Senha" onclick="alterarSenha(<?php echo $u->getIdUtilizador(); ?>, '<?php echo htmlspecialchars(addslashes($u->getUsername())); ?>')"><i class="bi bi-key"></i></button>
                                <button class="btn btn-warning btn-xs" title="Editar" onclick="abrirModalEdicao(<?php echo $u->getIdUtilizador(); ?>, '<?php echo htmlspecialchars(addslashes($u->getUsername())); ?>', '<?php echo $u->getNivel(); ?>', '<?php echo $u->getEstado(); ?>', '<?php echo $u->getIdFuncionario(); ?>')"><i class="bi bi-pencil"></i></button>
                                <button class="btn btn-danger btn-xs" onclick="abrirModalExcluir(<?php echo $u->getIdUtilizador(); ?>, '<?php echo htmlspecialchars(addslashes($u->getUsername())); ?>')"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if (count($lista) == 0): ?>
                        <tr><td colspan="5" style="text-align:center; padding:2rem; color:var(--text-muted);"><i class="bi bi-inbox" style="font-size:2rem; display:block; margin-bottom:0.5rem;"></i>Nenhum utilizador cadastrado.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal">
            ...
            <div class="modal-body">
                <form id="formUtilizador">
                    ...
                    <div class="form-group">
                        <label>ID Funcionário Associado</label>
                        <input type="number" id="idFuncionario" placeholder="ID do funcionário">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="btn" onclick="fecharModal('modalCadastro')" style="background:#e9ecef; color:var(--text-dark);">Cancelar</button>
                <button class="btn btn-primary" onclick="salvarUtilizador()"><i class="bi bi-check-lg"></i> Salvar</button>
            </div>
        </div>
    </div>
    ...
    <script>
        let utilizadorEstadoAtual = 'Ativo';
        ...
        function abrirModalCadastro() {
            ...
            utilizadorEstadoAtual = 'Ativo';
            document.querySelector('#modalCadastro h3').textContent = 'Novo Utilizador';
            document.getElementById('modalCadastro').classList.add('active');
        }

        function abrirModalEdicao(id, username, nivel, estado, idFuncionario) {
            document.getElementById('formUtilizador').reset();
            document.getElementById('utilizadorId').value = id;
            document.getElementById('username').value = username;
            document.getElementById('nivel').value = nivel;
            document.getElementById('idFuncionario').value = idFuncionario;
            utilizadorEstadoAtual = estado || 'Ativo';
            document.querySelector('#modalCadastro h3').textContent = 'Editar Utilizador';
            document.getElementById('modalCadastro').classList.add('active');
        }

        function abrirModalExcluir(id, username) {
            if (!confirm(`Eliminar o utilizador ${username}?`)) return;

            fetch('../../CONTROLLER/UtilizadorController.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'apagar', idUtilizador: id })
            })
            .then(r => r.json())
            .then(d => {
                if (d.success) {
                    mostrarToast('Utilizador eliminado com sucesso!');
                    setTimeout(() => location.reload(), 800);
                } else {
                    mostrarToast('Erro: ' + (d.message || 'Erro desconhecido'), true);
                }
            })
            .catch(() => mostrarToast('Erro de conexao com o servidor!', true));
        }
        ...
        function alterarSenha(id, username) {
            ...
            fetch('../../CONTROLLER/UtilizadorController.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'atualizarSenha', idUtilizador: id, senha: novaSenha })
            })
            .then(r => r.json())
            .then(d => {
                if (d.success) {
        ...
        function salvarUtilizador() {
            ...
            if (!document.getElementById('username').value || (!id && !senha)) {
                mostrarToast('Preencha os campos obrigatórios!', true); return;
            }

            const dados = {
                username: document.getElementById('username').value.trim(),
                nivel: document.getElementById('nivel').value,
                estado: utilizadorEstadoAtual,
                idFuncionario: parseInt(document.getElementById('idFuncionario').value) || 0
            };

            if (!id) {
                dados.senha = senha;
            } else {
                dados.idUtilizador = parseInt(id);
            }
            ...
            .then(d => {
                ...
                if (d.success) {
                    ...
                    mostrarToast(id ? 'Utilizador atualizado com sucesso!' : 'Utilizador criado com sucesso!');
                    setTimeout(() => location.reload(), 800);
                } else {
                    mostrarToast('Erro: ' + (d.message || 'Erro desconhecido'), true);
                }
        ...
        function mostrarToast(msg, erro = false) {
            ...
            t.className = 'toast ' + (erro ? 'error' : ''); t.classList.add('show');
