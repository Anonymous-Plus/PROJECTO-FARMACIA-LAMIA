...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li><a href="fornecedores.php" class="active"><i class="bi bi-truck"></i> Fornecedores</a></li>
   ...
   <script>
    ...
    function abrirModalEdicao(id, empresa, representante, telefone, email, endereco) {
        document.getElementById('modalTitulo').textContent = 'Editar Fornecedor #' + id;
...
function salvarFornecedor() {
    ...
    if (!empresa) {
        mostrarToast('Preencha o nome da empresa!', true);
        return;
    }
    ...
    if (id) {
        dados.idFornecedor = parseInt(id);
...
function excluirFornecedor(id) {
    enviarRequisicao('apagar', { idFornecedor: id }, 'excluído', true);
}
...
function enviarRequisicao(action, dados, acaoNome, isDelete = false) {
    ...
    fetch('../../CONTROLLER/FornecedorController.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            mostrarToast(`Fornecedor ${acaoNome} com sucesso!`);
            
            if (isDelete) {
                const row = document.querySelector(`tr[data-id="${dados.idFornecedor}"]`);
                if (row) row.remove();
                atualizarContador();
            } else {
...
function atualizarContador() {
    ...
    if (contador) {
        contador.textContent = `Todos os Fornecedores (${total})`;
    }
