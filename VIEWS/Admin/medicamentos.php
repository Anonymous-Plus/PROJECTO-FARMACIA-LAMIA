...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav"><ul>
            ...
            <?php if(in_array($nivel,['Administrador','Farmaceutico'])): ?><li><a href="relatorios.php"><i class="bi bi-graph-up"></i> Relatórios</a></li><?php endif; ?>
        </ul></nav>
...
<script>
    ...
    function abrirModalEdicao(id) {
        document.getElementById('modalTitulo').textContent = 'Editando Medicamento #' + id;
        document.getElementById('medId').value = id;
        document.getElementById('modalCadastro').classList.add('active');
        carregarDadosMedicamento(id);
    }

    function visualizarMedicamento(id) {
        fetch('../../CONTROLLER/MedicamentoController.php?action=buscar&id=' + id)
                ...
                if (data.success) {
                    ...
                    let html = `
                        <div style="padding: 1rem;">
                            <h3 style="color: var(--primary); margin-bottom: 0.5rem;">${m.nome}</h3>
                            <p style="color: var(--text-muted); margin-bottom: 1rem;">${m.descricao || 'Sem descrição'}</p>
                            <table style="width: 100%; border-collapse: collapse;">
                                <tr><td style="padding: 0.5rem; font-weight: 600;">Dosagem:</td><td>${m.dosagem}</td></tr>
                                <tr><td style="padding: 0.5rem; font-weight: 600;">Princípio Ativo:</td><td>${m.principioAtivo}</td></tr>
                                <tr><td style="padding: 0.5rem; font-weight: 600;">Preço Venda:</td><td>${Number(m.precoVenda).toFixed(2)} KZ</td></tr>
                                <tr><td style="padding: 0.5rem; font-weight: 600;">Stock:</td><td>${m.quantidadeEstoque}</td></tr>
                                <tr><td style="padding: 0.5rem; font-weight: 600;">Validade:</td><td>${new Date(m.dataValidade).toLocaleDateString('pt-BR')}</td></tr>
                            </table>
                        </div>`;
                    document.getElementById('conteudoVisualizar').innerHTML = html;
                } else {
                    document.getElementById('conteudoVisualizar').innerHTML = '<p style="text-align:center;padding:2rem;color:var(--danger);">Erro ao carregar dados.</p>';
                }
            })
            .catch(error => {
                console.error('Erro:', error);
                document.getElementById('conteudoVisualizar').innerHTML = '<p style="text-align:center;padding:2rem;color:var(--danger);">Erro de conexão.</p>';
            });
    ...
    function abrirModalExclusao(id, nome) {
        ...
        document.getElementById('btnConfirmarExclusao').onclick = () => excluirMedicamento(id);
    ...
    function salvarMedicamento() {
        ...
        if (!nome || !dataValidade) {
            mostrarToast('Preencha nome e data de validade!', true);
            return;
        }
        ...
        const dados = {
            ...
            estoqueMinimo: parseInt(document.getElementById('estoqueMinimo').value) || 10,
            dataFabricacao: document.getElementById('dataFabricacao').value,
            dataValidade: dataValidade,
            necessitaReceita: document.getElementById('necessitaReceita').value === '1' ? 'Sim' : 'Não',
            idCategoria: parseInt(document.getElementById('idCategoria').value) || 0,
            idFornecedor: parseInt(document.getElementById('idFornecedor').value) || 0
        };
        ...
        if (id) {
            // UPDATE
            dados.idMedicamento = parseInt(id);
            enviarRequisicao('atualizar', dados, 'atualizado');
        } else {
            // CREATE
            enviarRequisicao('cadastrar', dados, 'cadastrado');
        }
    }
    ...
    function excluirMedicamento(id) {
        enviarRequisicao('apagar', { idMedicamento: id }, 'excluído', true);
    }

    /**
     * CARREGAR DADOS PARA EDIÇÃO
     */
    function carregarDadosMedicamento(id) {
        fetch('../../CONTROLLER/MedicamentoController.php?action=buscar&id=' + id)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('precoCompra').value = m.precoCompra || 0;
                    document.getElementById('precoVenda').value = m.precoVenda || 0;
                    document.getElementById('quantidadeEstoque').value = m.quantidadeEstoque || 0;
                    document.getElementById('estoqueMinimo').value = m.estoqueMinimo || 10;
                    document.getElementById('dataFabricacao').value = m.dataFabricacao || '';
                    document.getElementById('dataValidade').value = m.dataValidade || '';
                    document.getElementById('necessitaReceita').value = m.necessitaReceita === 'Sim' ? '1' : '0';
                    document.getElementById('idCategoria').value = m.idCategoria || '';
                    document.getElementById('idFornecedor').value = m.idFornecedor || '';
                    
                    mostrarToast('Dados carregados com sucesso!');
                } else {
                    mostrarToast('Erro ao carregar dados: ' + (data.message || 'Erro desconhecido'), true);
                }
            })
            .catch(error => {
                console.error('Erro:', error);
                mostrarToast('Erro de conexão ao carregar dados!', true);
            });
    }
     ...
     */
    function enviarRequisicao(action, dados, acaoNome, isDelete = false) {
        // Mostra loading
        ...
        fetch('../../CONTROLLER/MedicamentoController.php', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                fecharModal(isDelete ? 'modalExclusao' : 'modalCadastro');
                mostrarToast(`Medicamento ${acaoNome} com sucesso!`);
                
                if (isDelete) {
                    // Remove a linha da tabela
                    const row = document.querySelector(`tr[data-id="${dados.idMedicamento}"]`);
                    if (row) row.remove();
                } else {
                ...
                mostrarToast('Erro: ' + (data.message || 'Erro desconhecido'), true);
            }
        })
        .catch(error => {
            console.error('Erro:', error);
    ...
    document.getElementById('buscaTopo').addEventListener('input', function() {
        document.getElementById('busca').value = this.value;
