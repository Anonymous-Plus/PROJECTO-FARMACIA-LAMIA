...
$paginaAtual = basename($_SERVER['PHP_SELF']); $titulo = 'Funcionários';
require_once '../../CONTROLLER/FuncionarioController.php';
$controller = new FuncionarioController();
$lista = $controller->listar()['data'] ?? [];
$totalSal = 0; foreach($lista as $f) $totalSal += $f->getSalario();
?>
<!DOCTYPE html>
<html lang="pt">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-icon.funcionarios { background: linear-gradient(135deg, #2c5f4b, #3d7a62); }
        .card-icon.salario { background: linear-gradient(135deg, #ffc107, #ffdb4d); }
        ...
        .modal { background: #fff; border-radius: 20px; width: 100%; max-width: 650px; box-shadow: 0 25px 60px rgba(0,0,0,0.3); transform: translateY(30px) scale(0.95); transition: all 0.4s cubic-bezier(0.175,0.885,0.32,1.275); max-height: 90vh; overflow-y: auto; }
        ...
        @media (max-width: 576px) { .topbar { padding: 1rem; } .topbar-search { display: none; } .page-content { padding: 1rem; } .form-row { grid-template-columns: 1fr; } .cards-grid { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li><a href="funcionarios.php" class="active"><i class="bi bi-person-badge"></i> Funcionários</a></li>
    ...
    <div class="main-content">
        ...
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="topbar-search">
                    ...
                    <input type="text" placeholder="Pesquisar funcionários..." id="buscaTopo">
                </div>
            </div>
        ...
        <div class="page-content">
            <div class="page-header">
                <div>
                    <h1><i class="bi bi-person-badge"></i> <?php echo $titulo; ?></h1>
                    <p>Gerir equipa da farmácia</p>
                </div>
                <button class="btn btn-primary" onclick="abrirModalCadastro()"><i class="bi bi-plus-lg"></i> Novo Funcionário</button>
            </div>
            ...
            <div class="cards-grid">
                <div class="card">
                    <div class="card-info">
                        <div class="card-number"><?php echo count($lista); ?></div>
                        <div class="card-label">Total Funcionários</div>
                    </div>
                    <div class="card-icon funcionarios"><i class="bi bi-person-badge"></i></div>
                </div>
                <div class="card">
                    <div class="card-info">
                        <div class="card-number"><?php echo number_format($totalSal, 0); ?> KZ</div>
                        <div class="card-label">Folha Salarial</div>
                    </div>
                    <div class="card-icon salario"><i class="bi bi-cash-stack"></i></div>
                </div>
            </div>
            ...
            <div class="table-container">
                <div class="table-header">
                    <h3>Todos os Funcionários (<?php echo count($lista); ?>)</h3>
                    <div class="search-box">
                        <input type="text" placeholder="Buscar funcionário..." id="busca" style="padding:0.5rem 1rem;border:1px solid var(--border);border-radius:20px;font-size:0.85rem;width:250px;outline:none;">
                    </div>
                </div>
                <table>
                    <thead>
                        <tr><th>Nome</th><th>Cargo</th><th>Telefone</th><th>Email</th><th>Salário</th><th>Admissão</th><th>Ações</th></tr>
                    </thead>
                    <tbody id="tabelaBody">
                        <?php foreach($lista as $f): ?>
                        <tr data-id="<?php echo $f->getIdFuncionario(); ?>">
                            <td><strong><?php echo htmlspecialchars($f->getNome()); ?></strong></td>
                            <td><span class="badge badge-info"><?php echo htmlspecialchars($f->getCargo()); ?></span></td>
                            <td><?php echo $f->getTelefone() ?: '—'; ?></td>
                            <td><?php echo $f->getEmail() ?: '—'; ?></td>
                            <td><?php echo number_format($f->getSalario(), 2); ?> KZ</td>
                            <td><?php echo $f->getDataAdmissao() ? date('d/m/Y', strtotime($f->getDataAdmissao())) : '—'; ?></td>
                            <td class="acoes">
                                <button class="btn btn-warning btn-xs" onclick="abrirModalEdicao(<?php echo $f->getIdFuncionario(); ?>, '<?php echo htmlspecialchars(addslashes($f->getNome())); ?>', '<?php echo $f->getSexo(); ?>', '<?php echo htmlspecialchars(addslashes($f->getCargo())); ?>', '<?php echo $f->getSalario(); ?>', '<?php echo htmlspecialchars(addslashes($f->getTelefone())); ?>', '<?php echo htmlspecialchars(addslashes($f->getEmail())); ?>', '<?php echo $f->getDataNascimento(); ?>', '<?php echo $f->getDataAdmissao(); ?>', '<?php echo htmlspecialchars(addslashes($f->getEndereco())); ?>')"><i class="bi bi-pencil"></i></button>
                                <button class="btn btn-danger btn-xs" onclick="abrirModalExclusao(<?php echo $f->getIdFuncionario(); ?>, '<?php echo htmlspecialchars(addslashes($f->getNome())); ?>')"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if (count($lista) == 0): ?>
                        <tr><td colspan="7" style="text-align:center; padding:2rem; color:var(--text-muted);"><i class="bi bi-inbox" style="font-size:2rem; display:block; margin-bottom:0.5rem;"></i>Nenhum funcionário cadastrado.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    <div class="modal-overlay" id="modalCadastro">
        <div class="modal">
            <div class="modal-header">
                <h3 id="modalTitulo">Novo Funcionário</h3>
            ...
            <div class="modal-body">
                <form id="formFuncionario">
                    ...
                    <div class="form-row">
                        <div class="form-group"><label>Nome <span style="color:var(--danger);">*</span></label><input type="text" id="nome" placeholder="Nome completo" required></div>
                        <div class="form-group"><label>Sexo</label><select id="sexo"><option value="">Selecionar</option><option value="Masculino">Masculino</option><option value="Feminino">Feminino</option></select></div>
                    </div>
                    <div class="form-row">
                        <div class="form-group"><label>Cargo</label><input type="text" id="cargo" placeholder="Ex: Farmacêutico"></div>
                        <div class="form-group"><label>Salário (KZ)</label><input type="number" id="salario" step="0.01" placeholder="0.00"></div>
                    </div>
                    <div class="form-row">
                        <div class="form-group"><label>Telefone</label><input type="text" id="telefone" placeholder="Ex: +244 923 456 789"></div>
                        <div class="form-group"><label>Email</label><input type="email" id="email" placeholder="funcionario@farmacia.ao"></div>
                    </div>
                    <div class="form-row">
                        <div class="form-group"><label>Data Nascimento</label><input type="date" id="dataNascimento"></div>
                        <div class="form-group"><label>Data Admissão</label><input type="date" id="dataAdmissao"></div>
                    </div>
                    <div class="form-group"><label>Endereço</label><input type="text" id="endereco" placeholder="Morada completa"></div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="btn" onclick="fecharModal('modalCadastro')" style="background:#e9ecef; color:var(--text-dark);">Cancelar</button>
                <button class="btn btn-primary" onclick="salvarFuncionario()"><i class="bi bi-check-lg"></i> Salvar</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay delete-modal" id="modalExclusao">
        <div class="modal">
            <div class="modal-body" style="padding:2.5rem;">
                ...
                <p>Tem certeza que deseja excluir o funcionário <strong id="nomeExcluir"></strong>?</p>
...
function abrirModalCadastro(){document.getElementById('modalTitulo').textContent='Novo Funcionário';document.getElementById('formFuncionario').reset();document.getElementById('funcionarioId').value='';document.getElementById('modalCadastro').classList.add('active')}

function abrirModalEdicao(id,nome,sexo,cargo,salario,telefone,email,dataNasc,dataAdm,endereco){
    ...
    document.getElementById('dataAdmissao').value=dataAdm;
    document.getElementById('endereco').value=endereco;
    document.getElementById('modalCadastro').classList.add('active');
}

function abrirModalExclusao(id,nome){document.getElementById('nomeExcluir').textContent=nome;document.getElementById('btnConfirmarExclusao').onclick=()=>excluirFuncionario(id);document.getElementById('modalExclusao').classList.add('active')}
function abrirModalLogout(){document.getElementById('modalLogout').classList.add('active')}
function fecharModal(id){document.getElementById(id).classList.remove('active')}

function excluirFuncionario(id){
    fetch('../../CONTROLLER/FuncionarioController.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({action: 'apagar', idFuncionario: id})
    })
    .then(r=>r.json())
    .then(d=>{
        if(d.success){
    ...
    .catch(e=>{console.error(e);mostrarToast('Erro de conexão!',true)});
}

function salvarFuncionario(){
    if(!document.getElementById('nome').value.trim()){mostrarToast('Preencha o nome!',true);return}
    ...
    if(id) dados.idFuncionario=parseInt(id);
    ...
    fetch('../../CONTROLLER/FuncionarioController.php',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({action: id?'atualizar':'cadastrar',...dados})
    })
    .then(r=>r.json())
    .then(d=>{
        btn.disabled=false;
    ...
    .catch(e=>{console.error(e);btn.disabled=false;btn.innerHTML='<i class="bi bi-check-lg"></i> Salvar';mostrarToast('Erro de conexão!',true)});
}
...
document.addEventListener('keydown',function(e){if(e.key==='Escape')document.querySelectorAll('.modal-overlay.active').forEach(m=>m.classList.remove('active'))});</script>
</body>
</html>
