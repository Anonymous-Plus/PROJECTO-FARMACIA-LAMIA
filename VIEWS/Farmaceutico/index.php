...
$totalFornecedores = $fornCtrl->contar()['total'] ?? 0;

$faturamento = 0; foreach($vendas as $v) $faturamento += $v->getValorTotal();

$stockBaixo = 0; $vencidos = 0;
foreach($medicamentos as $m) {
    if($m->getQuantidadeEstoque() < $m->getEstoqueMinimo()) $stockBaixo++;
    if(strtotime($m->getDataValidade()) < time()) $vencidos++;
}

$vendasHoje = 0; $hoje = date('Y-m-d');
foreach($vendas as $v) {
    if(date('Y-m-d', strtotime($v->getDataVenda())) == $hoje) $vendasHoje++;
}
?>
<!DOCTYPE html>
<html lang="pt">
<head>
    ...
    <title>Farmácia Lâmia | Painel Farmacêutico</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-title { font-size: 1.6rem; font-weight: 700; color: var(--text-dark); margin-bottom: 0.25rem; }
        ...
        .card-icon.fornecedores { background: linear-gradient(135deg, #6f42c1, #a855f7); }
        .card-icon.vencidos { background: linear-gradient(135deg, #fd7e14, #ffc107); }
        ...
        .btn { padding: 0.5rem 1rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; cursor: pointer; border: none; transition: var(--transition); text-decoration: none; display: inline-block; }
        ...
        .badge-status { padding: 0.25rem 0.6rem; border-radius: 12px; font-size: 0.75rem; font-weight: 600; }
        ...
        @media (max-width: 576px) { .page-content { padding: 1rem; } .cards-grid { grid-template-columns: 1fr 1fr; } }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="topbar-search"><i class="bi bi-search"></i><input type="text" placeholder="Pesquisar..."></div>
            </div>
            <div class="topbar-right">
                <div class="topbar-notification"><i class="bi bi-bell"></i><span class="badge"><?php echo $stockBaixo + $vencidos; ?></span></div>
        ...
        <div class="page-content">
            <h1 class="page-title"><i class="bi bi-grid-fill"></i> Dashboard</h1>
            <p class="page-subtitle">Bem-vindo, <?php echo htmlspecialchars($usuario['username']); ?>!</p>

            <div class="cards-grid">
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $totalMedicamentos; ?></div><div class="card-label">Medicamentos</div></div><div class="card-icon medicamentos"><i class="bi bi-capsule"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $vendasHoje; ?></div><div class="card-label">Vendas Hoje</div></div><div class="card-icon vendas"><i class="bi bi-cart4"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $totalClientes; ?></div><div class="card-label">Clientes</div></div><div class="card-icon clientes"><i class="bi bi-people-fill"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number" style="color:<?php echo $stockBaixo>0?'var(--danger)':'var(--success)'; ?>"><?php echo $stockBaixo; ?></div><div class="card-label">Stock Baixo</div></div><div class="card-icon stock"><i class="bi bi-exclamation-triangle"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number"><?php echo $totalFornecedores; ?></div><div class="card-label">Fornecedores</div></div><div class="card-icon fornecedores"><i class="bi bi-truck"></i></div></div>
                <div class="card"><div class="card-info"><div class="card-number" style="color:<?php echo $vencidos>0?'var(--danger)':'var(--success)'; ?>"><?php echo $vencidos; ?></div><div class="card-label">Vencidos</div></div><div class="card-icon vencidos"><i class="bi bi-calendar-x"></i></div></div>
            </div>

            <div class="table-container">
                <div class="table-header"><h3><i class="bi bi-exclamation-triangle-fill" style="color:var(--danger);"></i> Alertas de Stock</h3><a href="medicamentos.php" class="btn btn-primary">Ver Todos</a></div>
                <table><thead><tr><th>Nome</th><th>Stock</th><th>Mínimo</th><th>Validade</th><th>Status</th></tr></thead>
                    <tbody>
                        <?php $c=0; foreach($medicamentos as $m): if(($m->getQuantidadeEstoque()<$m->getEstoqueMinimo()||strtotime($m->getDataValidade())<time())&&$c<5): $c++;
                            $vencido=strtotime($m->getDataValidade())<time();
                            $cls=$vencido?'badge-danger':($m->getQuantidadeEstoque()==0?'badge-danger':'badge-warning');
                            $txt=$vencido?'Vencido':($m->getQuantidadeEstoque()==0?'Esgotado':'Baixo');
                        ?>
                        <tr><td><strong><?php echo htmlspecialchars($m->getNome()); ?></strong></td><td><?php echo $m->getQuantidadeEstoque(); ?></td><td><?php echo $m->getEstoqueMinimo(); ?></td><td><?php echo date('d/m/Y',strtotime($m->getDataValidade())); ?></td><td><span class="badge-status <?php echo $cls; ?>"><?php echo $txt; ?></span></td></tr>
                        <?php endif; endforeach; if($c==0): ?>
                        <tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--success);"><i class="bi bi-check-circle-fill" style="font-size:2rem;display:block;margin-bottom:0.5rem;"></i>Todos os medicamentos estão em ordem!</td></tr>
                        <?php endif; ?>
                    </tbody></table>
            </div>
        </div>
