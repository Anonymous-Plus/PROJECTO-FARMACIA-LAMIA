...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-primary:hover { background: var(--primary-dark); transform: translateY(-2px); box-shadow: 0 4px 15px rgba(44,95,75,0.3); }
        ...
        table tr:hover { background: rgba(44, 95, 75, 0.03); }
        ...
        @media (max-width: 576px) {
            ...
            table th, table td { padding: 0.6rem; font-size: 0.75rem; }
        }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="topbar-search">
                    ...
                    <input type="text" placeholder="Pesquisar clientes..." id="buscaTopo">
                </div>
            </div>
        ...
        <div class="page-content">
            <div class="page-header">
                <div>
                    <h1><i class="bi bi-people-fill"></i> <?php echo $titulo; ?></h1>
                    <p>Gerir clientes da farmácia</p>
                </div>
                <button class="btn btn-primary" onclick="abrirModalCadastro()"><i class="bi bi-plus-lg"></i> Novo Cliente</button>
            </div>

            <div class="table-container">
                <div class="table-header">
                    <h3>Todos os Clientes (<?php echo count($lista); ?>)</h3>
                    <div class="search-box">
                        <input type="text" placeholder="Buscar cliente..." id="busca" style="padding:0.5rem 1rem;border:1px solid var(--border);border-radius:20px;font-size:0.85rem;width:250px;outline:none;">
                    </div>
                </div>
                <table>
                    <thead>
                    ...
                    <tbody id="tabelaBody">
                        ...
                        <tr data-id="<?php echo $c->getIdCliente(); ?>">
                            <td><strong><?php echo htmlspecialchars($c->getNome()); ?></strong></td>
                            <td><?php echo $c->getSexo() ?: '—'; ?></td>
                            <td><?php echo $c->getTelefone() ?: '—'; ?></td>
                            <td><?php echo $c->getEmail() ?: '—'; ?></td>
                            <td><?php echo htmlspecialchars($c->getEndereco() ?: '—'); ?></td>
                            <td class="acoes">
                                <button class="btn btn-warning btn-xs" onclick="abrirModalEdicao(<?php echo $c->getIdCliente(); ?>, '<?php echo htmlspecialchars(addslashes($c->getNome())); ?>', '<?php echo $c->getSexo(); ?>', '<?php echo htmlspecialchars(addslashes($c->getTelefone())); ?>', '<?php echo htmlspecialchars(addslashes($c->getEmail())); ?>', '<?php echo htmlspecialchars(addslashes($c->getDataNascimento())); ?>', '<?php echo htmlspecialchars(addslashes($c->getEndereco())); ?>')"><i class="bi bi-pencil"></i></button>
                                <button class="btn btn-danger btn-xs" onclick="abrirModalExclusao(<?php echo $c->getIdCliente(); ?>, '<?php echo htmlspecialchars(addslashes($c->getNome())); ?>')"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <div class="modal-overlay" id="modalCadastro">
        <div class="modal">
                ...
                <form id="formCliente">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="telefone" placeholder="Ex: +244 923 456 789">
                        </div>
                        <div class="form-group">
                            <label>Email</label>
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" onclick="salvarCliente()"><i class="bi bi-check-lg"></i> Salvar</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay delete-modal" id="modalExclusao">
        <div class="modal">
            <div class="modal-body" style="padding:2.5rem;">
                ...
                <p>Tem certeza que deseja excluir o cliente <strong id="nomeExcluir"></strong>?</p>
    ...
    <script>
        ...
        function abrirModalCadastro(){clienteEmEdicao=null;document.getElementById('modalTitulo').textContent='Novo Cliente';document.getElementById('formCliente').reset();document.getElementById('clienteId').value='';document.getElementById('modalCadastro').classList.add('active')}
        async function carregarCliente(id){const resposta=await fetch(API_CLIENTE,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'buscar',idCliente:id})});const dados=await resposta.json();if(!dados.success)throw new Error(dados.message||'Não foi possível carregar o cliente.');return dados.data}
        async function abrirModalEdicao(id){try{const cliente=await carregarCliente(id);clienteEmEdicao=id;document.getElementById('modalTitulo').textContent='Editar Cliente';document.getElementById('clienteId').value=id;document.getElementById('nome').value=cliente.nome||'';document.getElementById('sexo').value=cliente.sexo||'';document.getElementById('telefone').value=cliente.telefone||'';document.getElementById('email').value=cliente.email||'';document.getElementById('dataNascimento').value=(cliente.dataNascimento||'').slice(0,10);document.getElementById('endereco').value=cliente.endereco||'';document.getElementById('modalCadastro').classList.add('active')}catch(erro){mostrarToast(erro.message||'Erro ao carregar cliente.',true)}}
        function abrirModalExclusao(id,nome){document.getElementById('nomeExcluir').textContent=nome;document.getElementById('btnConfirmarExclusao').onclick=()=>excluirCliente(id);document.getElementById('modalExclusao').classList.add('active')}
        function abrirModalLogout(){document.getElementById('modalLogout').classList.add('active')}
        function fecharModal(id){document.getElementById(id).classList.remove('active')}
        async function salvarCliente(){const nome=document.getElementById('nome').value.trim();if(!nome){mostrarToast('Preencha o nome do cliente!',true);return}const payload={action:clienteEmEdicao?'atualizar':'cadastrar',idCliente:clienteEmEdicao||'',nome,sexo:document.getElementById('sexo').value,telefone:document.getElementById('telefone').value.trim(),email:document.getElementById('email').value.trim(),dataNascimento:document.getElementById('dataNascimento').value,endereco:document.getElementById('endereco').value.trim()};try{const resposta=await fetch(API_CLIENTE,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const dados=await resposta.json();if(!dados.success)throw new Error(dados.message||'Não foi possível salvar o cliente.');fecharModal('modalCadastro');mostrarToast(dados.message||'Cliente salvo com sucesso!');setTimeout(()=>location.reload(),900)}catch(erro){mostrarToast(erro.message||'Erro de conexão com o servidor.',true)}}
        async function excluirCliente(id){try{const resposta=await fetch(API_CLIENTE,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'apagar',idCliente:id})});const dados=await resposta.json();if(!dados.success)throw new Error(dados.message||'Não foi possível excluir o cliente.');fecharModal('modalExclusao');mostrarToast(dados.message||'Cliente excluído com sucesso!');const row=document.querySelector(`tr[data-id="${id}"]`);if(row)row.remove()}catch(erro){mostrarToast(erro.message||'Erro de conexão com o servidor.',true)}}
        ...
        document.getElementById('busca').addEventListener('input',function(){const termo=this.value.toLowerCase();document.querySelectorAll('#tabelaBody tr').forEach(tr=>{tr.style.display=tr.textContent.toLowerCase().includes(termo)?'':'none'})});
