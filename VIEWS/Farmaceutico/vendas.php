<?php
session_start();
if (!isset($_SESSION['usuario']) || $_SESSION['usuario']['nivel'] !== 'Farmaceutico') {
...
foreach($vendas as $v) {
    if(date('Y-m-d', strtotime($v->getDataVenda())) == $hoje) {
        $vendasHoje++; $faturamentoHoje += $v->getValorTotal();
    }
}
$totalVendas = count($vendas);
$faturamentoTotal = 0; foreach($vendas as $v) $faturamentoTotal += $v->getValorTotal();
?>
<!DOCTYPE html>
<html lang="pt">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Farmácia Lâmia | Vendas</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:var(--bg);display:flex;min-height:100vh}
        ...
        .btn-success{background:var(--success);color:#fff}.btn-danger{background:var(--danger);color:#fff}.btn-info{background:var(--info);color:#fff}.btn-xs{padding:0.3rem 0.6rem;font-size:0.75rem}
        ...
        .card-number{font-size:1.8rem;font-weight:700;color:var(--text-dark)}.card-label{font-size:0.8rem;color:var(--text-muted)}
        .pdv-container{display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:2rem}
        .pdv-produtos,.pdv-carrinho{background:#fff;border-radius:var(--radius);padding:1.5rem;box-shadow:var(--shadow)}
        ...
        .lista-medicamentos{max-height:300px;overflow-y:auto}
        ...
        .carrinho-itens{max-height:250px;overflow-y:auto;margin-bottom:1rem}
        ...
        .carrinho-total{text-align:right;font-size:1.3rem;font-weight:700;color:var(--primary);padding-top:1rem;border-top:2px solid var(--border)}
        ...
        table th{background:var(--bg);font-weight:600;color:var(--text-muted);text-transform:uppercase;font-size:0.75rem}
        ...
        .modal{background:#fff;border-radius:20px;width:100%;max-width:500px;box-shadow:0 25px 60px rgba(0,0,0,0.3);transform:translateY(30px) scale(0.95);transition:all 0.4s}
        ...
        .modal-close{background:#f5f5f5;border:none;width:36px;height:36px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.3s}
        ...
        @media(max-width:992px){.sidebar{left:-260px}.sidebar.open{left:0}.main-content{margin-left:0}.menu-toggle{display:block}.pdv-container{grid-template-columns:1fr}.topbar-search{width:200px}}
    </style>
</head>
<body>
    ...
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-logo"><h2>FARMÁCIA LÂMIA</h2><span>Painel Farmacêutico</span></div>
    ...
    <div class="main-content">
        <header class="topbar">
            <div class="topbar-left"><button class="menu-toggle" id="menuToggle"><i class="bi bi-list"></i></button><div class="topbar-search"><i class="bi bi-search"></i><input type="text" placeholder="Pesquisar vendas..."></div></div>
        ...
        <div class="page-content">
            <div class="page-header"><h1><i class="bi bi-cart4"></i> Vendas</h1></div>
            ...
            <div class="cards-grid">
                ...
                <div class="card"><div class="card-number"><?php echo $totalVendas; ?></div><div class="card-label">Total Vendas</div></div>
                <div class="card"><div class="card-number"><?php echo number_format($faturamentoTotal,0); ?> KZ</div><div class="card-label">Faturamento Total</div></div>
            </div>

            <!-- PDV -->
            <div class="pdv-container">
                <div class="pdv-produtos">
                    <h3 style="margin-bottom:1rem;"><i class="bi bi-capsule"></i> Medicamentos</h3>
                    <div class="form-group"><input type="text" id="buscaMed" placeholder="Buscar..." oninput="filtrarMedicamentos()"></div>
                    <div class="lista-medicamentos" id="listaMedicamentos">
                        ...
                        <div class="med-item" onclick="adicionarAoCarrinho(<?php echo $m->getIdMedicamento(); ?>,'<?php echo htmlspecialchars(addslashes($m->getNome())); ?>',<?php echo $m->getPrecoVenda(); ?>)">
                            <div><strong><?php echo htmlspecialchars($m->getNome()); ?></strong><br><small style="color:var(--text-muted)">Stock: <?php echo $m->getQuantidadeEstoque(); ?></small></div>
                            <div style="color:var(--primary);font-weight:700;"><?php echo number_format($m->getPrecoVenda(),2); ?> KZ</div>
                        </div>
                        <?php endif; endforeach; ?>
                    </div>
                </div>
                <div class="pdv-carrinho">
                    <h3 style="margin-bottom:1rem;"><i class="bi bi-cart3"></i> Carrinho</h3>
            ...
            <div class="table-container">
                <div class="table-header"><h3>Histórico de Vendas</h3></div>
                <table><thead><tr><th>ID</th><th>Data</th><th>Valor</th><th>Pagamento</th><th></th></tr></thead>
                    <tbody><?php foreach($vendas as $v): ?>
                        <tr><td>#<?php echo $v->getIdVenda(); ?></td><td><?php echo date('d/m/Y H:i',strtotime($v->getDataVenda())); ?></td><td><strong><?php echo number_format($v->getValorTotal(),2); ?> KZ</strong></td><td><span class="badge badge-info"><?php echo $v->getFormaPagamento(); ?></span></td><td><button class="btn btn-info btn-xs" onclick="verDetalhes(<?php echo $v->getIdVenda(); ?>)"><i class="bi bi-eye"></i></button></td></tr>
    ...
    <!-- MODAL DETALHES -->
    <div class="modal-overlay" id="modalDetalhes"><div class="modal"><div class="modal-header"><h3>Detalhes da Venda</h3><button class="modal-close" onclick="fecharModal('modalDetalhes')"><i class="bi bi-x-lg"></i></button></div><div class="modal-body" id="conteudoDetalhes"></div></div></div>

    <!-- MODAL LOGOUT -->
    <div class="modal-overlay" id="modalLogout"><div class="modal" style="text-align:center;"><div class="modal-body" style="padding:2.5rem;"><div style="font-size:3rem;color:var(--danger);margin-bottom:1rem;"><i class="bi bi-box-arrow-right"></i></div><h4 style="color:var(--text-dark);">Terminar Sessão</h4><p style="color:var(--text-muted);">Tem certeza que deseja sair?</p><div style="display:flex;gap:1rem;justify-content:center;margin-top:2rem;"><button class="btn" onclick="fecharModal('modalLogout')" style="background:#e9ecef;">Cancelar</button><a href="../../logout.php" class="btn btn-danger" style="text-decoration:none;"><i class="bi bi-box-arrow-right"></i> Sair</a></div></div></div></div>

    <script>
        function abrirModalLogout(){document.getElementById('modalLogout').classList.add('active')}
        function fecharModal(id){document.getElementById(id).classList.remove('active')}
        function verDetalhes(id){document.getElementById('conteudoDetalhes').innerHTML='<p style="text-align:center;padding:2rem;"><i class="bi bi-receipt" style="font-size:2.5rem;color:var(--primary);display:block;margin-bottom:1rem;"></i>Venda #'+id+'</p>';document.getElementById('modalDetalhes').classList.add('active')}
        ...
        function finalizarVenda(){
            if(carrinho.length===0){mostrarToast('Adicione medicamentos!',true);return}
